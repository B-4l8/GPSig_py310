...
def signature_kern_first_order(M, num_levels, difference = True):
    ...
    if M.shape.ndims == 4: 
        ...
        K = [tf.ones((num_examples1, num_examples2), dtype=gpflow.default_float())]
    else:
        num_examples, len_examples = tf.shape(M)[0], tf.shape(M)[1]
        K = [tf.ones((num_examples), dtype=gpflow.default_float())]
...
def signature_kern_higher_order(M, num_levels, order=2, difference = True):
    ...
    for i in range(2, num_levels+1):
        ...
        for j in range(2, d+1):
            R_next[0, j-1] = 1 / tf.cast(j, gpflow.default_float()) * M * tf.cumsum(tf.add_n(R[:, j-2].tolist()), exclusive=True, axis=1)
            R_next[j-1, 0] = 1 / tf.cast(j, gpflow.default_float()) * M * tf.cumsum(tf.add_n(R[j-2, :].tolist()), exclusive=True, axis=-1)
            for k in range(2, d+1):
                R_next[j-1, k-1] = 1 / (tf.cast(j, gpflow.default_float()) * tf.cast(k, gpflow.default_float())) * M * R[j-2, k-2]
...
def tensor_kern(M, num_levels):
    ...
    K = [tf.ones((num_tensors, num_tensors2), dtype=gpflow.default_float())]
...
def signature_kern_tens_vs_seq_first_order(M, num_levels, difference = True):
    ...
    K = [tf.ones((num_tensors, num_examples), dtype=gpflow.default_float())]
...
def signature_kern_tens_vs_seq_higher_order(M, num_levels, order=2, difference = True):
    ...
    for i in range(1, num_levels+1):
        ...
        for j in range(1, i):
            ...
            for l in range(1, d):
                R_next[l] = 1. / tf.cast(l+1, gpflow.default_float()) * M[k] * R[l-1]
            R = R_next
            k += 1
...
def signature_kern_first_order_lr_feature(U, num_levels, rank_bound, sparsity = 'sqrt', seeds = None, difference = True):
    ...
    Phi = [tf.ones((num_examples, 1), dtype=gpflow.default_float())]
...
def tensor_kern_lr_feature(U, num_levels, rank_bound, sparsity = 'sqrt', seeds = None):
    ...
    Phi = [tf.ones((num_tensors, 1), dtype=gpflow.default_float())]
